            ...
            if (posts != null) {
              for(Post p : posts) { %>
                <div class="chat flex flex-col w-1/2 my-2<% if (p.getAuthor().getId() == user.getId()) { %> justify-self-end items-end <% } %>">
                  ...
                  <div class="chat-bubble font- text-clip break-words p-2 rounded-lg <% if (p.getAuthor().getId() == user.getId()) { %> bg-linear-to-tr from-blue-200 from-10% via-blue-300 via-70 to-blue-400 to-100% before:bg-transparent<% }  else { %> bg-gray-300 <% } %>">
                    <p><%= p.getContent() %></p>
                  </div>
                </div>
          <% } } %>
